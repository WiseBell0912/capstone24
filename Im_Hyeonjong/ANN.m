...
load_ADCP %date_w01 / Hs_w01
%date_w01a = datetime(date_w01,'convertFrom','datenum');
date_w01a = datenum(date_w01); %뭐지... 위에꺼 안되서 대체함
Hs_w01a = Hs_w01;
...
date_w01b = datenum(date_w01b); %내가 추가
Hs_w01b = data(:,1);

date_w01 = [date_w01a',date_w01a(end)+1, date_w01b'];
Hs_w01 = [Hs_w01a',nan, Hs_w01b'];

Hs_w01_tr = interp1(date_w01,Hs_w01,rrdate,'linear');
...
idx = ~isnan(Hs_w01_tr);
Hs_w01_tr = Hs_w01_tr(idx); %ADCP & AWAC Hs
date_w01_tr = rrdate_(idx); %ADCP & AWAC Date (double)

rrdate = datenum(rrdate_); %wave parameter Date (double)
hs_tr = hs_gpd_tr; %bouy Hs -> 보간해서 wave parameter Date랑 같아짐
...
%Tps = movingA(Tp,bb);
Tps = movmean(Tp, [2, 0], 'omitnan'); %기존 것에서 내장 함수로 전환
...
divDtSet = 1/2; % = 12시간 double 형태

Ddate = (rrdate(2)-rrdate(1)); % 20분 double형태
divDidx = ceil(divDtSet/Ddate); % 12시간 / 20분 --> 36개
divDt = divDidx*Ddate; % 36개 * 20분 = 12시간
divN = floor(length(rrdate) / divDidx); % = 2158
