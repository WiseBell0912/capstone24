...
[~, ~, raw, dates] = xlsread([pwd, '\train_data\안인파랑관측_201909-202001.to.Gopher.xlsx'],'Sheet1','A2:D2664','',@convertSpreadsheetExcelDates);
raw = raw(:,[2,3,4]);
dates = dates(:,1);
...
ANN_SIMULATE_INPUT = [Tp',Tp(idx2)',Tp(idx3)',...
    ...
    energy_logic(idx1)', energy_logic(idx2)', energy_logic(idx3)',...
    energy_tr(idx1)', energy_tr(idx2)', energy_tr(idx3)']';
...
Pdir = Pdir(1:sizOri);
SNR = SNR(1:sizOri);
energy_logic = energy_logic(1:sizOri);
...
% xlim(xlim_set)
xlim([datetime('2020-01-31') datetime('2020-02-20')])
ylim([0 5])
...
y = net(ANN_TRAINING_INPUT);
plotregression(hs_tr,y,'Regression')
